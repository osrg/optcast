...
ncclResult_t ncclSockDevices(int* ndev) {
  INFO(NCCL_ALL, "HI! ncclSockDevices");
  *ndev = ncclNetIfs;
  return ncclSuccess;
}
...
ncclResult_t ncclSockInit(ncclDebugLogger_t logFunction) {
  if (ncclNetIfs == -1) {
      ...
      if (ncclNetIfs <= 0) {
        ...
        for (int i=0; i<ncclNetIfs; i++) {
          ...
          snprintf(line+strlen(line), MAX_LINE_LEN-strlen(line), " [%d]%s:%s", i, names+i*MAX_IF_NAME_SIZE,
              ncclSocketToString(&addrs[i], addrline, 1));
        }
        line[MAX_LINE_LEN] = '\0';
        INFO(NCCL_INIT|NCCL_NET,"SOCKext/Socket : Using%s", line);
      }
    }
...
ncclResult_t ncclSockListen(int dev, void* opaqueHandle, void** listenComm) {
  ...
  comm = malloc(sizeof(struct ncclNetSocketListenComm));
  memset(comm, 0, sizeof(struct ncclNetSocketListenComm));
...
ncclResult_t ncclSockConnect(int dev, void* opaqueHandle, void** sendComm) {
  if (dev < 0 || dev >= ncclNetIfs) { // data transfer socket is based on specified dev
    return ncclInternalError;
  }
  ...
  *sendComm = NULL;
  ...
  comm = malloc(sizeof(struct ncclNetSocketComm));
  memset(comm, 0, sizeof(struct ncclNetSocketComm));
  stage->comm = comm;
...
ncclResult_t ncclSockAccept(void* listenComm, void** recvComm) {
  ...
  *recvComm = NULL;
  ...
  rComm = malloc(sizeof(struct ncclNetSocketComm));
  memset(rComm, 0, sizeof(struct ncclNetSocketComm));
  stage->comm = rComm;
...
//  CUDACHECK(cudaGetDevice(&rComm->cudaDev));
  for (; i<rComm->nSocks+1; i++) {
    ...
    sock = malloc(sizeof(struct ncclSocket));
    memset(sock, 0, sizeof(struct ncclSocket));

    NCCLCHECK(ncclSocketInit(sock, NULL, NCCL_SOCKET_MAGIC, ncclSocketTypeUnknown, NULL, 0));
    stage->sock = sock;
...
ncclResult_t ncclNetSocketGetTask(struct ncclNetSocketComm* comm, int op, void* data, int size, struct ncclNetSocketTask** req) {
  ...
  if (queue->tasks == NULL) {
    ...
    queue->tasks = malloc(sizeof(struct ncclNetSocketTask)*queue->len);
    memset(queue->tasks, 0, sizeof(struct ncclNetSocketTask)*queue->len);
    queue->next = 0;
    res->comm = comm;
...
ncclResult_t ncclSockTest(void* request, int* done, int* size) {
  *done = 0;
  ...
  if (r->used == 1) { /* try to send/recv size */
    ...
    if (r->op == NCCL_SOCKET_RECV && data > r->size) {
      ...
      WARN("NET/Socket : peer %s message truncated : receiving %d bytes instead of %d. If you believe your socket network is in healthy state, \
          ...
          ncclSocketToString(&addr, line, 1), data, r->size);
    ...
    if (r->comm->nSocks > 0) {
      ...
      int taskSize = MAX(MIN_CHUNKSIZE, DIVUP(r->size, r->comm->nSocks));
...
ncclResult_t ncclSockRegMr(void* comm, void* data, int size, int type, void** mhandle) {
  return (type != NCCL_PTR_HOST) ? ncclInternalError : ncclSuccess;
}

ncclResult_t ncclSockDeregMr(void* comm, void* mhandle) {
  return ncclSuccess;
}

ncclResult_t ncclSockIsend(void* sendComm, void* data, int size, int tag, void* mhandle, void** request) {
...
ncclResult_t ncclSockIrecv(void* recvComm, int n, void** data, int* sizes, int* tags, void** mhandles, void** request) {
...
ncclResult_t ncclSockIflush(void* recvComm, int n, void** data, int* sizes, void** mhandles, void** request) {
  // We don't support CUDA pointers, so we don't need a flush operation
  return ncclSuccess;
}

ncclResult_t ncclSockClose(void* opaqueComm) {
...
ncclResult_t ncclSockCloseListen(void* opaqueComm) {
...
const ncclNet_v6_t socketPlugin_v6 = {
  .name = "SOCKext",
  .init = ncclSockInit,
...
const ncclNet_v5_t socketPlugin_v5 = {
  .name = "SOCKext",
  .init = ncclSockInit,
  .devices = ncclSockDevices,
  .getProperties = ncclSockGetProperties,
